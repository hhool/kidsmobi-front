...
/**
 * Canonical slug for each resolved product category. Prerendered category
 * pages live at /products/<slug>/ and the sitemap lists those URLs, so the
 * hydrated SPA must canonicalize every category variant (id-form, alias
...
const resolveRouteState = (pathname: string, hash: string): RouteState => {
  ...
  if (root === "products") {
    ...
    } else if (secondSegment && contentSegments[2]) {
      // /products/<bucket>/<product-id> where the bucket is a resolved (but not
      // nav-primary) category, e.g. /products/other/kids_tricycles-b0fh23snxm
      // generated by resolveProductCategoryId. Without this branch the second
      // segment was misread as a product id and detail pages 404'd.
      activeProductCategory = normalizedCategory || "all";
      activeProductId = contentSegments[2];
...
export default function App() {
  ...
  useEffect(() => {
   ...
   // keywordsArr = applyPageKeywordOverride(seoKey, keywordsArr, lang, activeProductCategory);
    ...
    // Category pages canonicalize onto the prerendered slug-form URL
    // (/products/<slug>/) so id-form and alias variants share one canonical
    ...
    const canonicalPath = productCategoryCanonicalSlug
      ...
      : buildCanonicalPathFromPageConfig(pageConfig, currentPath, activePageIndex);
                              ...
                              {productsMegaMenuCategories.map((cat) => (
                                <button
                                  ...
                                  onClick={() => {
                                    ...
                                    if (activeTab === "products" && activeProductCategory === cat.id) {
                                      const element = document.getElementById("expert-picks-anchor");
                                      if (element) {
                                        element.scrollIntoView({ behavior: "smooth", block: "start" });
                                      }
