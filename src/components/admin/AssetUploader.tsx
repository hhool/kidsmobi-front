...
export default function AssetUploader({ lang = "zh", onUploaded }: { lang?: "zh" | "en", onUploaded?: (url: string, key: string) => void }) {
  ...
  const kindOf = (asset: any): "image" | "video" | "pdf" | "other" => {
  ...
  const { folders, currentFiles, totalCount, totalBytes } = useMemo(() => {
    ...
    assets.forEach(asset => {
      ...
      if (query && !asset.key.toLowerCase().includes(query)) return;
      if (typeFilter !== "all" && kindOf(asset) !== typeFilter) return;
    ...
    return { folders: Array.from(folderSet).sort(), currentFiles: fileList, totalCount: count, totalBytes: bytes };
  }, [assets, currentPath, search, typeFilter]);
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-4 bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
        ...
        <div className="flex flex-wrap items-center gap-2 shrink-0">
          <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 bg-slate-50 border border-slate-100 px-3 py-1.5 rounded-lg">
            {totalCount} {lang === "zh" ? "个资源" : "assets"} · {(totalBytes / 1024 / 1024).toFixed(1)} MB
          </span>
          <div className="relative">
            ...
            <input
              ...
              placeholder={lang === "zh" ? "搜索文件名…" : "Search…"}
              className="bg-slate-50 border border-slate-200 pl-8 pr-3 py-1.5 rounded-lg text-xs font-bold outline-none focus:border-orange-400 w-36"
            />
          </div>
          <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200">
            {([
              { value: "all", label: lang === "zh" ? "全部" : "All" },
              { value: "image", label: lang === "zh" ? "图片" : "Img" },
              { value: "video", label: lang === "zh" ? "视频" : "Video" },
              { value: "pdf", label: "PDF" },
            ] as const).map((item) => (
              <button
                key={item.value}
                ...
                className={`px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider transition ${
                  typeFilter === item.value ? "bg-white shadow-sm text-slate-900" : "text-slate-400"
                }`}
              >
                {item.label}
              </button>
